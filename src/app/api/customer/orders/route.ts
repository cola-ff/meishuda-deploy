...
export async function GET(request: Request) {
  try {
    ...
    const orders = await prisma.order.findMany({
      ...
      include: { items: { include: { model: { include: { product: true } } } } }
    })
...
export async function POST(request: Request) {
  try {
    const { customerId, items, customerNote } = await request.json()
    ...
    const order = await prisma.order.create({
      data: { customerId, totalAmount, customerNote, createdBy: 'customer', items: { create: itemData } },
